...
\end{itemize}
\solution{S'ennuyer, ça s'entraîne}
\begin{enumerate}
	\item \begin{itemize}
...
\solution{Fonctions à paramètres par défaut}

\textbf{Code :} \detokenize{solutions/part2/chapter3/default_parameters.c}
...
\subsection{Listes}
\textbf{REMARQUE (IMPORTANTE) : } l'implantation de la signature est faite \textit{telle quelle}. Dans une implantation réelle\footnote{\textit{i.e.} industrielle}, on ajoute soit des \textit{static inline} soit on évite d'implanter des routines d'une seule ligne, qui sont jolies d'un point de vue théorique mais absolument immonde et non optimisé dans la pratique. C'est une limitation du C qui laisse peu de libertés sur la programmation d'opérateurs (à comparer au C++).
