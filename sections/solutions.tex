...
avec la première inégalité grâce au \textsf{break}.
\inputminted{c}{solutions/eratosthene.c}

\textbf{Question 2 :}

Si on utilise l'algorithme de l'\refexercise{Test de primalité} pour calculer les nombres premiers inférieurs à $N$, on obtient un algorithme pour lister les nombres premiers en $\displaystyle\underset{2\nmid i}{\sum_{i = 3}^{N}}\sqrt{i} \underset{n\rightarrow+\infty}{\sim}\dfrac{2}{3}N^{\frac{3}{2}}$ tours de boucle, par comparaison série-intégrale. 

Comparons avec l'algorithme précédent : d'après le résultat admis, $\pi(N)>\dfrac{N}{ln(N)}$.

\textbf{Question 3 :}

On utilise pour le test de primalité les deux idées :
\inputminted{c}{solutions/list_primal_optimized.c}
