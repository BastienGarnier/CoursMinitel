...
\begin{document}
\textit{\underline{Recommandation} : revoir le vocabulaire sur les fichiers de la partie 1 du cours (section \ref{sec:architecture_logicielle})}.
...
Voir les annales d'\textit{Algorithme et Programmation 1}. Toutes les notions nécessaires ont été abordées (voir l'\refexercise{Listes chainées} de la section \ref{sec:structures} pour les listes chaînées). Une correction par Minitel (et par les profs (beurk !)) sera proposé aussi vite que possible sur le NextCloud.
\end{document}