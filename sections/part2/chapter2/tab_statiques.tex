...
\exercise{Routines classiques de manipulation de tableaux}
...
\end{enumerate}
\exercise{Liste des nombres premiers}
\begin{enumerate}
	...
	\item \textbf{(difficile)}Démontrer que l'algorithme qui parcourt tous les entiers inférieurs à $N$ et leur applique le test de primalité de l'\refexercise{Test de primalité} (en $\mathcal{O}(\sqrt(N))$) est asymptotiquement plus rapide que l'algorithme trouvé ci-avant.
	\item Déduire des deux algorithmes un nouvel algorithme plus rapide que les deux précédents.\footnote{La démonstration n'est pas exigée.}
\end{enumerate}
\textit{
	\underline{Remarque :} On pourra librement utiliser dans cet exercice les résultats suivants ($\pi(x)$ est le nombre de nombres premiers inférieurs ou égaux à $x$) :
	\begin{itemize}
