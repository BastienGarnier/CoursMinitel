...
\hline
\end{tabular}
\captionof{table}{Priorités des opérateurs en C}\label{tab:priorite_operateurs}
\end{center}
...
\definition{Type booléen et valeur de vérité d'une expression}{
Le type abstrait \textit{booléen} est déterminé par l'ensemble $\{Vrai, Faux\}$ et des opérations logiques $\vee$, $\wedge$, $\oplus$ ou $\neg$.
...
On dit alors qu'une expression numérique (à valeur dans $\mathbb{R}$) est \textit{fausse} si sa valeur est égale à 0. On dit qu'elle est \textit{vraie} sinon.
}
...
\begin{itemize}
	\item \mintinline{c}{++variable}\footnote{Ce qui a d'ailleurs amené au nommage du langage \textit{C++} comme une version ``incrémentée'', étendue, du langage C}
	\item \mintinline{c}{--variable}
\end{itemize}
...
Ainsi, \mintinline{c}{++a} renvoie la valeur de $a$ \textit{après} incrémentation et \mintinline{c}{--a}  renvoie la valeur de $a$ \textit{après} décrémentation.
...
\exercise{Valeur absolue (1)}{18}En utilisant uniquement des opérations arithmétiques et bit-à-bits, écrire un programme qui calcule la valeur absolue d'un entier signé :
\begin{minted}{c}
