...
\end{enumerate}
\begin{algorithm}
\caption{Parcours de liste\label{alg:exercice_parcours_liste}}
\KwIn{$Liste:l$, $Procedure:p$}
\textit{Noeud} $tmp \leftarrow tete(l)$\;
\While {$successeur(tmp)$ existe} {
	$p(contenu(tmp))$\;
	$tmp \leftarrow successeur(tmp)$\;
}
\end{algorithm}
