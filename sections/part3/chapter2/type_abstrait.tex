...
\exercise{Entiers de taille quelconque}{25} En représentant les nombres de $\mathbb{Z}$ sur un nombre d'octets déterminé dynamiquement selon la taille des éléments, implanter une structure de données \textit{Integer} de l'anneau $\mathbb{Z}$ muni de l'addition et de la multiplication. Les choix de \textit{design} et d'implantation sont laissés à la liberté du lecteur.

\exercise{Nombres rationnels}{15} Implanter le type abstrait du corps $\mathbb{Q}$ muni de l'addition et de la multiplication en représentant chaque élément comme un couple $(a, b)\in\mathbb{Z}\times{\mathbb{N}}$ \newline
\textit{\underline{Remarque :} on pourra utiliser la structure construite à l'\refexercise{Entiers de taille quelconque}}
\end{document}